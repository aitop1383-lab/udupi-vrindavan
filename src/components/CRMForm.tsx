...
import { Utensils, ShieldCheck, Loader2 } from "lucide-react";
...
const CRMForm = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Constructing URL parameters as Google Apps Script handles POST requests 
    // more reliably via query params or FormData in some configurations.
    const scriptURL = "https://script.google.com/macros/s/AKfycbwwG3Z_KW5NAF22aMhnASbcJ0ZG2r0RhXmprv-XbYz4r_1gxXSN-zkM9lSVs0updLvInw/exec";

    try {
      // Note: Posting to Google Scripts often requires 'no-cors' mode 
      // if you don't have control over the server headers.
            ...
            {status === "success" ? (
              ...
              >
                ...
                <button
                  type="submit"
                  disabled={status === "loading"}
                  className="w-full bg-brand-blue text-brand-cream py-5 md:py-6 rounded-xl md:rounded-2xl font-bold text-lg md:text-xl hover:bg-brand-gold transition-all shadow-xl hover:shadow-brand-gold/30 disabled:opacity-50 mt-4 flex items-center justify-center gap-3"
                >
                  {status === "loading" ? (
                    <>
